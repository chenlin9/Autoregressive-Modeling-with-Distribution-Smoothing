...
class ReversePixelCNNPPTrainRunner(object):
    ...
    def train(self):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for batch_idx, (input, _) in enumerate(train_loader):
                ...
                noisy_input = input + torch.randn_like(input) * self.config.noise
                clean_input = input + torch.randn_like(input) * self.config.clean_noise # add very small noise
            ...
            with torch.no_grad():
                for batch_idx, (input_var, _) in enumerate(test_loader):
                    ...
                    noisy_input_var = input_var + torch.randn_like(input_var) * self.config.noise
                    clean_input_var = input_var + torch.randn_like(input_var) * self.config.clean_noise #* 0.02

                    input_var = torch.cat([noisy_input_var, clean_input_var], dim=2)
                    output = test_model(input_var)[:, :, input_var.shape[-1]:, :]
            ...
            if epoch % 10 == 0:
                ...
                sample_t = torch.cat([clean_input_var[:25], sample_t], dim=2) #add original sample
